...
*Update December 2020:* This appears to work on macOS Big Sur, using the latest Raspbian. Config also now uses HFS+ as the drive filesystem and SMB as the network protocol, which I previously could not get to work (had to rely on AFP and ext4).
...
- Install dependencies: `sudo apt install samba avahi-daemon`
...
- edit samba config: `/etc/avahi/services.samba.service` and append
...
- Finder > Connect to server
    - Enter `smb://192.168.1.50/` (use the local static IP for your pi instead though)
    - You should be prompted for your pi login info

![Connect to server](./connect-to-smb.png)
...
![Time Machine](./time-machine-progress.png)
